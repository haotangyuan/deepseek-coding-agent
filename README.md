...
- 首次进入包含项目 AGENTS/Skills/Prompts、`.pi` 配置或 `.deepseek-code/validation.json` 的陌生目录时先禁用项目资源，并在 TUI 中展示来源后选择本次/永久启用或禁用；一次性命令保持 fail-closed。
...
`/verify` 优先读取已信任且已启用的 `.deepseek-code/validation.json`，可列出多个命名检查；`/verify <name>` 只预览精确命令，不调用模型也不执行命令。没有项目配置时，才从 `package.json`、`pyproject.toml`、`Cargo.toml`、`go.mod` 等已知入口推导一个保守候选。只有看过候选与费用提示后输入 `/verify confirm`，才会新增一次 Agent 回合；该回合要求只运行展示过的精确命令且不修改文件，Bash 仍按现有策略审批。
...
配置最多 20 条命令；名称只能使用小写字母、数字、`:`、`_`、`-`，命令必须是单行。文件未信任时产品只发现它的存在，不读取内容；格式错误、超限或符号链接逃逸会明确报错且不会静默回退。关闭 `/resources` 或撤销信任会清除已预览命令。Pi 会限制长 Bash 输出，并把被截断的完整输出保存在本机临时文件。详细边界见 [docs/verification-loop.md](docs/verification-loop.md)。

`/cache` 直接读取 Pi 已归一化的 DeepSeek usage：本轮通过提交前后 SessionStats 做差，Session 数值包含 JSONL 全历史的实际计费 usage。它不会为了诊断缓存再发送请求，也不会猜测命中下降原因。

`/context` 展示当前有效 System Prompt 的字符数和粗略 token、活动工具、资源数量与项目信任状态；`/agents`、`/skills`、`/prompts` 展示真实来源路径和作用域。未知项目默认不加载项目/祖先 AGENTS、项目 Skills/Prompts、项目 `.pi` 设置，也不读取项目验证配置，TUI 会先列出来源并要求输入 `y/a/n/d`。`/trust once|always|off|deny` 可重新选择，`/resources on` 不能绕过未信任状态。用户级资源始终保留，第三方 Extension 仍禁用，项目信任也不会改变工具审批模式。
...
`--runs` 最多 5 次；默认观测成本上限为 0.02 美元，达到上限后不会开始下一次请求。单次请求的最终成本只能在 Provider 返回 usage 后得知，因此该参数不是预付费硬限额；超过上限会在汇总中标记失败。Pro 必须用 `--model deepseek-v4-pro` 显式选择。Schema v3 的 dry-run 同时显示逻辑样本数、最大 Provider 请求数和 `promptProfile`；真实执行每个样本输出 `eval_result`，最后输出包含按任务通过率、延迟、成本和工具错误的 `eval_summary`。任务、格式和真实 smoke 见 [docs/deepseek-evaluation.md](docs/deepseek-evaluation.md)，高区分度 Prompt A/B 见 [docs/high-discrimination-profile-ab.md](docs/high-discrimination-profile-ab.md)。

7 个 repair 任务会在系统临时目录创建有缺陷的极小项目，覆盖单文件、多文件、配置解析、跨模块契约、长 CI 日志和验证失败恢复。评测器只自动批准 fixture 内的 write/edit，拒绝 Bash；Agent 结束后由评测器运行测试，确认指定源码确实改变、原文件没有缺失、受保护文件未变且没有创建额外文件，然后删除整个临时目录。`repair-feedback` 和 `repair-validation` 的隐藏回归测试位于 Agent 工作区外：失败后只回填脱敏、截断且不含测试路径/堆栈的摘要，最多再执行一次 60 秒修复尝试；`repair-validation` 第一次完整修复也会正确记为通过。
