...
Agent 模式决定模型可见的能力边界，审批模式决定这些能力的执行授权。两者不互相替代。

| Agent 模式 | 含义 |
|---|---|
| `plan` | 只暴露 read/ls/grep；策略钩子也拒绝 write/edit/bash |
| `build` | 按审批模式暴露工具；默认模式 |

| 审批模式 | Build 可见工具 | Plan 可见工具 | 行为 |
|---|---|---|---|
| `ask` | read/ls/grep/write/edit/bash | read/ls/grep | 只读工具自动允许；Build 中 write/edit/bash 逐次确认 |
| `auto-read` | read/ls/grep | read/ls/grep | 只允许工作区内读取、列目录和内容搜索 |
| `deny` | 无 | 无 | 不向模型暴露工具 |

默认组合是 `build + ask`。非 TTY 环境无法安全交互确认，因此 write/edit/bash 自动拒绝，不会假定同意。

`ask` 下的 write/edit 始终逐次确认。Bash 可选择允许一次，或仅在当前进程记住完全相同的命令字符串；没有通配符/前缀匹配，不写入 Session JSONL。危险命令和敏感路径检查先执行，因此不能被已有命令授权绕过。
...
flowchart TD
    ...
    Supported -->|是| AgentMode{"agent mode"}
    AgentMode -->|plan + mutation| Block
    AgentMode -->|otherwise| Mode{"approval mode"}
    Mode -->|deny| Block
...
工作区内部还应用敏感路径规则：`.env` 及非模板变体、常见凭据文件/目录和 SSH 私钥名默认拒绝；`.env.example/.sample/.template` 允许作为公开配置模板。read/ls/grep/write/edit 均在文件读取或修改前检查，明显在 Bash 命令中出现的敏感路径字面量也会在审批前阻断。完整集合和 Shell 绕过边界见 `docs/sensitive-paths.md`。
...
当前自动化测试共 55 个，其中工具策略覆盖：

- 三种模式和 read/ls/grep/write/edit/bash allowlist。
- Plan/Build 与三种审批模式的组合矩阵、热切换，以及 Plan 对伪造修改调用的二次阻断。
- 敏感文件/目录拒绝、公开 `.env` 模板放行，以及 Bash 明显路径引用在审批前阻断。
- 精确 Bash 命令的进程内复用，以及任意命令变化后重新审批。
