...
| 模式 | 模型可见工具 | 行为 |
|---|---|---|
| `ask` | read/write/edit/bash | read 自动允许；write/edit/bash 逐次确认 |
| `auto-read` | read | 只允许工作区内读取 |
| `deny` | 无 | 不向模型暴露工具 |
...
flowchart TD
    ...
    Mode -->|deny| Block
    Mode -->|auto-read| ReadOnly{"read?"}
    ReadOnly -->|否| Block
    ReadOnly -->|是| Path
    Mode -->|ask| Path["文件路径边界检查"]
    Path -->|越界| Block
    Path -->|允许| Risk{"工具风险"}
    Risk -->|read| Execute["执行 Pi 原工具"]
...
本项目继续使用 Pi 的 read/write/edit/bash 实现，没有复制文件编辑、diff 应用或进程执行逻辑。

## 5. 文件路径边界

read/write/edit 在审批前执行两层检查：
...
自动化测试共 18 个，其中 M2 覆盖：

- 三种模式和工具 allowlist。
- 工作区内读取、`../` 越界与 symlink 逃逸。
