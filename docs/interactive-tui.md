...
- tool running/done/failed 卡片；Bash 额外显示命令、cwd、持续时间、退出码、超时、取消和 Pi 截断信息。
...
| `/tool [id-prefix]` | 展开或折叠最近一次/指定工具卡的参数与结果 tail |
...
| `/settings [approval <mode>]` | 查看持久偏好，或保存下一次启动的 approval 默认值 |
| `/trust [once\|always\|off\|deny]` | 查看或改变当前项目上下文信任；always/deny 会记住选择 |
...
`/model`、`/thinking`、`/mode` 和 `/reasoning` 的变化会写入产品私有设置，显式 CLI 参数仍覆盖保存值。`/settings approval ...` 只改变下次启动，避免运行中静默替换已创建的 ToolPolicy。
...
首次进入包含项目指令、Skills、Prompts 或 `.pi` 项目配置的未知目录时，输入区先切换为信任选择卡：`y` 本次启用、`a` 启用并记住、`n` 本次禁用、`d` 禁用并记住。在决定前普通任务不会发送给模型。未信任状态下 `/resources on` 不能绕过该边界；一次性 CLI 不弹交互而保持项目资源禁用。

信任只决定文本上下文和 Pi 项目设置是否加载。第三方 Extension 始终禁用，write/edit/bash 仍按 ToolPolicy 审批；信任文件也不保存 API Key。
...
- reasoning 默认折叠和历史展开。
- tool start/end 卡片。
- Bash 流式 update 保留原始命令；成功、非零退出、超时、取消、截断和 `/tool` 展开均有覆盖。
- Provider 错误分类、自动重试 recovered/exhausted 卡片。
- write 审批接受。
- Bash 允许一次、当前进程精确授权和拒绝结果。
- DeepSeek 模型与 thinking 切换。
- 首次项目上下文信任、普通输入阻断、临时/持久选择，以及未信任时 `/resources on` 拒绝。
- 用户设置保存、显式启动值与损坏设置安全回退。
...
- 工具结果支持最多 16 行 tail 展开，但没有结果内搜索；完整长 Bash 输出仍由 Pi 保存到卡片显示的本机临时路径。
- UI 使用固定轻量 ANSI 配色，没有主题系统。
- `/context` 使用字符数除以 4 粗估 System Prompt token，不替代 Provider tokenizer。
- 临时信任/禁用只影响当前进程；remembered 决策按项目真实路径持久化。
- 当前项目级产品偏好不提供自定义验证命令覆盖；`/verify` 仍只从固定 manifest 发现候选。
